...
class Policy extends Admin_gic_core
{
	...
	public function filter_policy()
	{
		...
		$filter_prev_policy_no = $this->input->post("filter_prev_policy_no");
		...
		$filter_sub_agency_code = $this->input->post("filter_sub_agency_code");
		...
		if (!empty($this->input->post())) {
			if (!empty($filter_year) && !empty($filter_month) && !empty($filter_day)) {
				$whereArr["DATE_FORMAT(policy_member_details.date_from,'%d-%m-%Y')"] = $filter_day . "-" . $filter_month . "-" . $filter_year;
			} elseif (!empty($filter_year) && !empty($filter_month)) {
				$whereArr["DATE_FORMAT(policy_member_details.date_from,'%m-%Y')"] = $filter_month . "-" . $filter_year;
			} elseif (!empty($filter_day) && !empty($filter_month)) {
				$whereArr["DATE_FORMAT(policy_member_details.date_from,'%d-%m')"] = $filter_day . "-" . $filter_month;
			} elseif (!empty($filter_year) && !empty($filter_day)) {
				$whereArr["DATE_FORMAT(policy_member_details.date_from,'%d-%Y')"] = $filter_day . "-" . $filter_year;
			} else {
				...
				if (!empty($filter_day))
					$whereArr["DATE_FORMAT(policy_member_details.date_from,'%d')"] = $filter_day;
			}
			...
			if (!empty($filter_policy_no))
				$likeCondtnArr["policy_member_details.policy_no"] = $filter_policy_no;
			if (!empty($filter_prev_policy_no))
				$likeCondtnArr["policy_member_details.pre_year_policy_no"] = $filter_prev_policy_no;
			...
			if (!empty($filter_policy_type))
				$whereArr["policy_member_details.policy_type"] = $filter_policy_type;

			if (!empty($filter_agency))
				$whereArr["policy_member_details.fk_agency_id"] = $filter_agency;
			if (!empty($filter_sub_agency_code))
				$whereArr["policy_member_details.fk_sub_agent_id"] = $filter_sub_agency_code;
			if (!empty($filter_group_name))
				$whereArr["policy_member_details.fk_client_id"] = $filter_group_name;
			...
			$total_policy_data = count($result2);
	...
	function get_last_serial_no($serial_no_year, $serial_no_month)
	{
		// SELECT MAX(CONVERT(id, SIGNED INTEGER)) + 1 as tp FROM customermem_tbl order by id desc;
		$policy_counter = $this->db->query('SELECT MAX(CONVERT(policy_counter, SIGNED INTEGER)) as policy_counter FROM policy_member_details where serial_no_year = ' . $serial_no_year . ' and serial_no_month = ' . $serial_no_month . ' order by policy_counter desc')->row()->policy_counter;
		// print_r($maxid );
		// die();
	...
	public function addpolicy()
	{
		...
		// $yesterday_date = date($date,strtotime("-5 days"));
		// print_r($yesterday_date);
		// print_r($lastMonth);
		// print_r($nextMonth);
		// print_r($date);
		// $date_from = "2021-07-03";
		// $date_to = date("Y-m-d");
		// $date_from = date('Y-m-d', strtotime($date_from. ' - 1 days'));
		// $date_from = date('Y-m-d', strtotime($date_from. ' + 1 days'));
		// $date_from = date('2021-07-03',strtotime("-1 days"));
		// $date_to = date($date_to,strtotime("-1 days"));
		// print_r($date_from);
		// print_r($date_to);
		// die();
		$validator = array('status' => false, 'messages' => array());
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->input->post() != "") {
				...
				$floater_policy_type = "";
				$sub_policy_name_txt_box = "";
				$floater_policy_type = $this->security->xss_clean($this->input->post('floater_policy_type'));
				$sub_policy_name_txt_box = $this->security->xss_clean($this->input->post('sub_policy_name_txt_box'));

				if (!empty($sub_policy_name_txt_box))
					$floater_policy_type = $sub_policy_name_txt_box;
				...
				if ($mode_of_premimum > 4) {
					...
					// print_r($mode_of_premimum."mode_of_premimum");
					// print_r($new_mode_of_pre."new_mode_of_pre");
					// die();
					$new_policy_id = "";
					for ($i = 1; $i <= $counter; $i++) {
						$add_arr = array();
						...
						// else
						// 	$serial_no_year = $serial_no_year + 1;

						// $counter_whereArr["policy_member_details.serial_no_year"] = $serial_no_year;
						// $counter_query = $this->Mquery_model_v3->getListRecordsQuery($tableName = "policy_member_details", $colNames = "policy_member_details.policy_id , policy_member_details.policy_counter , policy_member_details.serial_no,policy_member_details.serial_no_year", $whereArr = $counter_whereArr, $whereInArray = array(), $customWhereArray = array(), $orWhereArray = array(), $orWhereDataArr = array(), $likeCondtnArr = array(), $joinArr = array(), $orderByArr = array(), $groupByArr = array(), $singleRow = True, $colActive = TRUE, $paginationArr = "");

						// $fetch_serialno_result = $counter_query["userData"];
						// $policy_counter = $fetch_serialno_result["policy_counter"];
						// $this->load->model("Mcommon");
						// $policy_counter = $this->get_last_serial_no($serial_no_year,$serial_no_month);
						// $policy_counter = $this->Mcommon->get_policy_counter($serial_no_year, $serial_no_month);
						// $policy_counter = $policy_counter["policy_counter"];
						// print_r($policy_counter);
						...
						// print_r($policy_counter_new);
						// echo $this->db->last_query();

						// $policy_counter = $this->get_last_serial_no($serial_no_year = $new_date_from_year,$serial_no_month=$date_from[1]);
						// $policy_counter_length = strlen((string)$policy_counter);
						// $policy_counter_length = $policy_counter_data;
						...
						// else
						// 	$date_to[0] = $date_to[0]+1;
						$date_to = $date_to[0] . "-" . $date_to[1] . "-" . $date_to[2];
						// print_r($new_date_from_year."<br/>");

						if ($i > 1) {
							$date_from = date('Y-m-d', strtotime($date_from . ' - 1 days'));
							$date_to = date('Y-m-d', strtotime($date_to . ' - 1 days'));
							// $date_from = date($date_from,strtotime("-1 days"));
							// $date_to = date($date_to,strtotime("-1 days"));
						}
						// print_r($date_from."<br/>");
						// print_r($date_to);
						// die();
						$add_arr[$i] = array(
							'serial_no_year' => $serial_no_year,
								...
								if (!empty($policy_name_txt)) {
									if (($policy_name_txt == "Worksmen Compentation") || ($policy_name_txt == "Money In Transit") || ($policy_name_txt == "Plate Glass") || ($policy_name_txt == "House Holder") || ($policy_name_txt == "All Risk") || ($policy_name_txt == "Office Compact") || ($policy_name_txt == "Electronic Equipment") || ($policy_name_txt == "Product Liability") || ($policy_name_txt == "Commercial General Liability") || ($policy_name_txt == "Public Liability") || ($policy_name_txt == "Professional Indemnity") || ($policy_name_txt == "Machinery Breakdown") || ($policy_name_txt == "Contractors All Risk")) {
										if ($policy_type != 3) {
											...
											$query2 = $this->Mquery_model_v3->addQuery($tableName = "others_policy", $add_other_policy_arr, $return_type = "lastID");
											$burglary_policy_id = $query2["lastID"];
										}
									}
								}
					...
					// explode(",",$new_policy_id);
					// print_r($new_policy_id);
					// die("Test");
	...
	public function get_policy_list()
	{
		...
		$joins_main[] = array("tbl" => "master_agency", "condtn" => "policy_member_details.fk_agency_id = master_agency.magency_id", "type" => "left");
		$joins_main[] = array("tbl" => "master_sub_agent", "condtn" => "policy_member_details.fk_sub_agent_id = master_sub_agent.sub_agent_id", "type" => "left");
		$query = $this->Mquery_model_v3->getListRecordsQuery($tableName = "policy_member_details", $colNames = "policy_member_details.policy_id, policy_member_details.serial_no, policy_member_details.serial_no_year, policy_member_details.serial_no_month, policy_member_details.fk_company_id,policy_member_details.fk_department_id , policy_member_details.fk_policy_type_id , policy_member_details.fk_client_id , policy_member_details.fk_cust_member_id , policy_member_details.fk_agency_id , policy_member_details.fk_sub_agent_id , policy_member_details.policy_type , policy_member_details.mode_of_premimum ,DATE_FORMAT(policy_member_details.date_from,'%d-%m-%Y') as date_from , DATE_FORMAT(policy_member_details.date_to,'%d-%m-%Y') as date_to , policy_member_details.payment_date_from , policy_member_details.payment_date_to , policy_member_details.policy_no , policy_member_details.pre_year_policy_no , policy_member_details.date_commenced , policy_member_details.policy_upload ,policy_member_details.dynamic_path , policy_member_details.reg_mobile , policy_member_details.reg_email , policy_member_details.policy_member_status , policy_member_details.del_flag , policy_member_details.basic_sum_insured , policy_member_details.basic_gross_premium , policy_member_details.current_sum_insured , policy_member_details.current_total_premium , policy_member_details.plan_policy_commission , policy_member_details.commission_rece_company , policy_member_details.commission_amt_rece_company, policy_member_details.renewal_flag, policy_member_details.commission_flag, policy_member_details.endorsment_flag, policy_member_details.claims_flag , policy_member_details.riv , policy_member_details.type_of_bussiness , policy_member_details.description_of_stock , policy_member_details.quotation_date , policy_member_details.quotation_upload , policy_member_details.quotation_male_link ,policy_member_details.floater_policy_type, master_company.company_name, master_department.department_name, master_policy_type.policy_type as policy_type_title,CONCAT(customermem_tbl.name, ' ', REPLACE(customermem_tbl.middle_name,'null',''),' ',REPLACE(customermem_tbl.surname,'null','')) AS member_name, master_agency.code as master_agency_code, master_agency.name as master_agency_name, master_sub_agent.sub_agent_code, master_sub_agent.sub_agent_name , customer_tbl.grpname", $whereArr = array(), $whereInArray = array(), $customWhereArray = array(), $orWhereArray = array(), $orWhereDataArr = array(), $likeCondtnArr = array(), $joinArr = $joins_main, $orderByArr = array(), $groupByArr = array("policy_member_details.policy_id"), $singleRow = FALSE, $colActive = TRUE, $paginationArr = "");
		$result = $query["userData"];
		$total_policy_data = count($result);
		// print_r($result);	pre_year_policy_no	'current_sum_insured' => $current_sum_insured,'current_total_premium' => $current_total_premium,
		// die("Test");
		if (!empty($result)) {
			$result["status"] = true;
			$result["userdata"] = $result;
	...
	public function download_doc()
	{
		...
		if ($doc_type != "") {
			...
			elseif ($doc_type == 2) {
				...
				$data = file_get_contents("./policy_document/quotation_upload/" . $doc_name);
			}
		}
	...
	public function view_doc()
	{
		...
		if (!empty($doc_type)) {
			if ($doc_type == 1) {
				...
				$extension = pathinfo("policy_document/quotation_upload/" . $doc_name, PATHINFO_EXTENSION);
				$file = file_get_contents("./policy_document/quotation_upload/" . $doc_name);
			}
			// print_r($extension);
			// die("Test");
	...
	public function delete_doc()
	{
		...
		$query = $this->Mquery_model_v3->getListRecordsQuery($tableName = "policy_member_details", $colNames =  "policy_member_details.policy_id, policy_member_details.serial_no, policy_member_details.fk_company_id,policy_member_details.fk_department_id , policy_member_details.fk_policy_type_id , policy_member_details.fk_client_id , policy_member_details.fk_cust_member_id , policy_member_details.fk_agency_id , policy_member_details.fk_sub_agent_id , policy_member_details.policy_type , policy_member_details.mode_of_premimum , policy_member_details.date_from , policy_member_details.date_to , policy_member_details.payment_date_from , policy_member_details.payment_date_to , policy_member_details.policy_no , policy_member_details.date_commenced , policy_member_details.policy_upload ,policy_member_details.dynamic_path,policy_member_details.quotation_upload  , policy_member_details.reg_mobile , policy_member_details.reg_email , policy_member_details.policy_member_status , policy_member_details.del_flag", $whereArr = array("policy_member_details.policy_id" => $id), $whereInArray = array(), $customWhereArray = array(), $orWhereArray = array(), $orWhereDataArr = array(), $likeCondtnArr = array(), $joinArr = array(), $orderByArr = array(), $groupByArr = array(), $singleRow = True, $colActive = TRUE, $paginationArr = "");
		$this->data["policy_doc_details"] = $policy_doc_details = $query["userData"];
		$file_name = $policy_doc_details["policy_upload"];
		$policy_upload = $file_name;
		$dynamic_path = $policy_doc_details["dynamic_path"];
		$dynamic_path_new = $dynamic_path;
		$quotation_upload = $policy_doc_details["quotation_upload"];
		if (!empty($doc_type)) {
			if ($doc_type == 1) {
				$title = "Policy Upload Document";
				$file_name = $policy_doc_details["policy_upload"];
				$dynamic_path = $policy_doc_details["dynamic_path"];
				$policy_upload = "";
				$dynamic_path_new = "";
				...
				$title = "Quotation Upload Document";
				$quotation_upload = "";
				$extension = pathinfo("policy_document/quotation_upload/" . $doc_name, PATHINFO_EXTENSION);
				$file = file_get_contents("./policy_document/quotation_upload/" . $doc_name);
				unlink("./policy_document/quotation_upload/" . $doc_name);
			}
			$updateArr[] = array(
				"policy_id" => $id,
				"dynamic_path" => $dynamic_path_new, //1:Running, 0:Deleted
				"policy_upload" => $policy_upload, //1:Running, 0:Deleted
				"quotation_upload" => $quotation_upload, //1:Running, 0:Deleted
				"fk_staff_id" => $this->session->userdata("@_staff_id"),
				"fk_user_role_id" => $this->session->userdata("@_user_role_id"),
			);
			$whereArr["policy_member_details.policy_id"] = $id;
		...
		echo json_encode($result);
		// redirect("master/policy/view_policy/" . $id, "refresh");
	}
		...
		if ($this->input->post() != "") {
			...
			if (!empty($policy_name_txt)) {
				if ($policy_name_txt == "Bharat Sookshma Udyam") {     		//Bharat Sookshma Udyam
					...
					} elseif (($company == "Max Bupa Health Insurance Co Ltd") || ($company == "Niva Bupa Health Insurance Co Ltd")) {
						if ($floater_policy_type == "Reassure") {
					...
					if ($company == "HDFC ERGO HEALTH INSURANCE LTD" || $company == "HDFC Ergo General Insurance Co Ltd") {
						...
						if ($floater_policy_type == "Star Super Surplus (Super Top Up)") {
						...
						if ($floater_policy_type == "Uiic Super Top Up") {
