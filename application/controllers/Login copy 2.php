		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->input->post() != "") {
				...
				if (!empty($username)) {
					...
					// print_r($ip_address_arra_data);die();
				...
				if (empty($result)) {
					...
					// $insertLogArr[] = array(
					// 	"fk_user_role_id" => $staff_name_user_role_id,
					// 	"fk_staff_id" => $staff_name_id,
					// 	"user_name" => $username,
					// 	"password" => $password,
					// 	"log_date" => date("Y-m-d"),
					...
					if (!empty($global_ip_address_btn_status)) {
						if ($global_ip_address_btn_status == 1) { // 1:Global , 2:Local
							...
							if (in_array($ip_address, $ip_address_arra_data)) {
								...
								// $this->session->set_userdata("@_user_role_id", $user_role_id);
								// $this->session->set_userdata("@_user_role_title", $user_role_title);
								// print_r($user_role_id_session);die();
								if ($result["fk_user_role_id"] == 1 || $result["fk_user_role_id"] == 2) {
									...
									// print_r($result["fk_user_role_id"]);die();
									$this->session->sess_destroy();
									$this->clear_cache();
