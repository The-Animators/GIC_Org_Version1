	...
	public function auth()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->input->post() != "") {
				...
				if (empty($result)) {
					$validator["status"] = false;
					$validator["messages"] = "User Name & Password doesn't Matches.";
					echo json_encode($validator);
					die();
				} else {
					if($check_role == '_2'){
				...
				if ($result["status"] == 1) {
					$insertLogArr[] = array(
						// "fk_user_role_id" => $result["role_type"],
						"member_id" => $result["id"],
						"log_type" => 1,
					...
					$insertLogArr[] = array(
						// "fk_user_role_id" => $result["fk_access_level_id"],
						"member_id" => $result["id"],
						"log_type" => 2,
					...
					if ($output1["status"] == FALSE) {
						$validator["status"] = false;
						$validator["messages"] = "Something Went Wronge.";
						echo json_encode($validator);
						die();
					}
				}
