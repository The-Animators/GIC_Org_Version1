...
<div class="row">
    ...
    <div class="col-md-12" style="overflow-x:auto;">
        <table class="table mb-0 table-bordered" id="first_table">
            <thead>
                <tr>
                    <th><button class="btn btn-primary btn-sm Add_Care_Advantage" id="Add_Care_Advantage" onclick="AddCareAdvantage(0)">Add</button></th>
    ...
    <div class="col-md-12">
        <table class="table mb-0 table-bordered mt-2 col-md-12">
                ...
                <td><strong id="medi_final_premium_td"><input type="text" id="medi_final_premium" name="medi_final_premium" class="form-control" disabled><input type="hidden" id="care_adv_ind_id" name="care_adv_ind_id" class="form-control"></td>
            </tr>
        </table>
    </div>
</div>
...
<script>
    ...
    function AddCareAdvantage(add_care_adv_counter) {
                ...
                success: function(data) {
                    if (data["status"] == true) {
                        ...
                        if (dob == "null") {
                            ...
                            $('#name_insured_sum_insured_' + add_care_adv_counter).prop('selectedIndex',0);
                            $('#ncd_type_' + add_care_adv_counter).prop('selectedIndex',0);
                            $('#maternity_type_' + add_care_adv_counter).prop('selectedIndex',0);
                            $('#limit_of_cataract_type_' + add_care_adv_counter).prop('selectedIndex',0);
                            $('#limit_of_cataract_prem_' + add_care_adv_counter).val("");
                            $('#ncd_prem_' + add_care_adv_counter).val("");
                            $('#maternity_prem_' + add_care_adv_counter).val("");
                            $('#basic_premium_' + add_care_adv_counter).val("");
                            $('#tot_prem_' + add_care_adv_counter).val("");
                            return false;
                        } else {
                            dob = dateFormate(dob);
                        }
    ...
    function check_suminsured(add_care_adv_counter){

        var name_insured_age = $("#name_insured_age_" + add_care_adv_counter).val();
        var name_insured_sum_insured = $("#name_insured_sum_insured_" + add_care_adv_counter).val();
    ...
    function get_basic_premium(add_care_adv_counter) {
        $('#basic_premium_' + add_care_adv_counter).val("");
        // $('#medi_final_premium').val("");
        check_suminsured(add_care_adv_counter);
        Tot_basic_premium();
        ...
        if (name_insured_age != "" && name_insured_sum_insured != "null"  && name_insured_sum_insured != "" && condition_value != "" && column_value !="") {
            var url = "<?php echo base_url(); ?>master/remote/get_care_health_care_advantage_ind_basic_prem";
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data["status"] == true) {
                            ...
                            $('#basic_premium_' + add_care_adv_counter).val("");
                            $('#basic_premium_' + add_care_adv_counter).val(data["userdata"]);
