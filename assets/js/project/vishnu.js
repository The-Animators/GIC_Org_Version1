...
function getPackingType(flag){
    if(flag == true){
        DrawDataTable('#packing_typeTable','packing_type','VishnulenePackingType', 'Vishnulene Packing Type List',[0,2,3,4,5],[0,1],[0,1],[3,4]);
    }
    getPackingTypeList();
}
...
function getItemPacking(flag){
    if(flag == true){
        DrawDataTable('#item_packingTable','item_packing','VishnuleneItemPacking', 'Vishnulene Item Packing List',[0,2,3,4],[0,1],[0,1],[3], false,"2,3");
    }
    getItemPackingList();
    getItemPackingType();
}
    ...
    $.ajax({
        ...
        success : function(resData){
            ...
            if(status === true){
                ...
                $('#'+formId+' [id="notes"]').val(data[0].notes)
...
function getItems(flag){
    if(flag == true){
        DrawDataTable('#itemsTable','items','VishnuleneItems', 'Vishnulene Items List',[0,2,3,4],[0,1],[0,1]);
    }
    getItemsList();
}
...
function getPriceList(flag){
    ...
    //  DrawDataTable(table_id,file_name, title,show_columns,url,orderable,center_align, right_align)
        DrawDataTable('#price_listTable','price_list','VishnulenePricing', 'Vishnulene Pricing List',[0,3,4,5,6,7,8,9,10],[0,1,2],[0,1,2,3,4],[7,9],true,"3,6,7",'landscape');
    }
}
...
function getItemPackingList(){
    ...
    $.ajax({
        ...
        success : function(resData){
            ...
            if(status === true){
                var len = data.length;
                $("#item_packing_id").append("<option selected data-cost='a' data-psc='a' data-cut='a' value=''>Select Vishnu Item Packing</option>");
                for( var i = 0; i<len; i++){
                    var id = data[i]['id'];
...
function calculatePP(){
    ...
    // console.log('pp: '+pp+' -- mcut: '+mcut+' -- pp_cut: '+pp_cut);
    if(cut != '3.00'){
